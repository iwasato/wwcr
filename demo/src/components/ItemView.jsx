				...
				return(<div className="outer" style={this.state.visibleItemIdList.indexOf(item.id) == -1 ? style.invisible : {}}>
					...
					<label onClick={this._before_onclick} for={'item-'+item.id} item-id={item.id} className="inner">
						<div className="uppercontent">
							...
							<img src={item.iconurl} className={this.state.downercontent || item.downercontent ? "invisible" : "icon"}></img>
							<div style={this.state.downercontent || item.downercontent ? style.label.long : style.label.short}>
								<div className="item-label">{item.label}</div>
								{(()=>{
									switch(item.id[0]){
										case 'k':
										return (<div class="item-sub-label blue">safari</div>)
										break;
										case 's': case 'a': case 'i':
										return (<div class="item-sub-label green">Mados</div>)
										break;
										default:
										return (<div class="item-sub-label red">Power Point</div>)
										break;
									}
								})()}
							</div>
						...
						<div className="downercontent" style={this.state.downercontent || item.downercontent ? style.downercontent : style.invisible}>
							<img class="image" src={item.screenurl}></img>
						</div>
					</label>
				</div>)
			})
