...
window.onload = ()=>{
	...
	domPack['create-classroom__button'].onclick = (e)=>{
		swal({
			...
			// const value = bridge.sendSyncToMain('create-new-classroom',swal._value);
			// if(value.result){
			// 	CLASSROOMLIST.push({
			// 		name: swal._value['name'],
			// 		token: swal._value['token'],
			// 		ownerId: swal._value['ownerId'],
			// 		ownerName: swal._value['ownerName'],
			// 		color: swal._value['color'],
			// 		id: value.roomId
			// 	});
			// 	classroomView.setClassroomList(CLASSROOMLIST);
			// }
