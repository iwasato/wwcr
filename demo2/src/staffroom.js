...
window.onload = ()=>{
	...
	buttonPack['navigation-toptool__button'].onclick = ()=>{
		...
		.then(value=>{
			switch(value){
				...
				case 'bookmark': //
				setTimeout(()=>{
					componentPack['item-view'].setAllVideo(democontents.video['bookmark']);
				},3000);
				break;
				case 'wwc-go-to-page':
				...
				.then(value=>{
					setTimeout(()=>{
						componentPack['item-view'].setAllVideo(democontents.video['gotopage']);
					},3000);
				});
				break;
				case 'presenter'://
				...
				case 'autoplay'://
				setTimeout(()=>{
					componentPack['item-view'].setAllVideo(democontents.video['autoplay']);
				},3000);
				break;
				case 'backmessage': //
	...
	buttonPack['lock-toptool__button'].onclick = ()=>{
		setTimeout(()=>{
			componentPack['item-view'].setAllVideo(democontents.video['lock']);
		},3000)
	}
	...
	buttonPack['navigation-itemtool__button'].onclick = (e)=>{
		...
		.then(value=>{
			switch(value){
				...
				case 'audience'://
				setTimeout(()=>{
					componentPack['item-view'].setVideo(focusItemId,democontents.video['mode-change']);
				},3000);
				break;
				case 'free'://
...
const userToItem = (user)=>{
	return {
		...
		screenurl: user.id=='tora' ? democontents.video['presen-init'] : democontents.video['audien-init'],
		checked: false,
		downercontent: false
	}
}
